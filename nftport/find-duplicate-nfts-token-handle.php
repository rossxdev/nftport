...
if($_POST['submit'] == 'Submit') {
	$response_array = find_duplicate_nfts_token($_POST['chain'],$_POST['contract_address'],$_POST['token_id'],$_POST['page_number'],$_POST['page_size'],$_POST['threshold'],$_POST['filter_out_contract_address']);
}
//printR($_POST);
//printR($response_array['similar_nfts'][0]);
/*
[chain] => polygon
...
<body>
	<?include 'nav.php'; ?>
	<?if($response_array['response']=='OK' AND $response_array['is_similar']=='1') {?>
		<div class="container">
			...
			<div class="container">
				<div id="search-container" class="row row-cols-1 row-cols-sm-2 row-cols-md-5 g-3" data-masonry='{"percentPosition": true }'>
					<?
				    foreach($response_array['similar_nfts'] as $nft_array) {
				      ...
				      ?>
						...
						<a target="_blank" href="get-nft-details?contract_address=<?=$token_address?>&token_id=<?=$token_id?>&chain=<?=$chain?>" type="button" class="btn btn-sm btn-outline-secondary">NFT</a>

						<a target="_blank" href="get-contract-nfts?contract_address=<?=$token_address?>&chain=<?=$chain?>" type="button" class="btn btn-sm btn-outline-secondary">Contract</a>
